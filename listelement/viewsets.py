...
class ElementViewSet(viewsets.ModelViewSet):
    ...
    def url(self, request):
        print(request.query_params['url_clean'])
        """ queryset = Element.objects.get(
            url_clean=request.query_params['url_clean']) """
        queryset = get_object_or_404(
            Element, url_clean=request.query_params['url_clean'])
...
class CategoryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def url(self, request):
        queryset = get_object_or_404(
            Category, url_clean=request.query_params['url_clean'])
...
class TypeViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def url(self, request):
        queryset = get_object_or_404(
            Type, url_clean=request.query_params['url_clean'])
